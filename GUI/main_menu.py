...
class MainMenu:
    def __init__(self, app_frame, signup, update_user, add_book, update_book, view_books, search, report):
        ...
        self.update_user.place(relx=0.7, rely=0.35, anchor=ctk.CENTER)

        self.add_book = ctk.CTkButton(self.menu, text="Add Book", command=add_book)
        self.add_book.place(relx=0.3, rely=0.5, anchor=ctk.CENTER)

        self.update_book = ctk.CTkButton(self.menu, text="Update Book", command=update_book)
        self.update_book.place(relx=0.7, rely=0.5, anchor=ctk.CENTER)

        self.view_books = ctk.CTkButton(self.menu, text="View Books", command=view_books)
        self.view_books.place(relx=0.3, rely=0.65, anchor=ctk.CENTER)
        
        self.search_books = ctk.CTkButton(self.menu, text="Search Books", command=search)
        self.search_books.place(relx=0.7, rely=0.65, anchor=ctk.CENTER)

        self.report = ctk.CTkButton(self.menu, text="Report", command=report)
        self.report.place(relx=0.5, rely=0.8, anchor=ctk.CENTER)
